...
export async function POST(req: Request) {
  ...
  let body: { audioData?: unknown; mimeType?: unknown };
  try {
    body = await req.json() as { audioData?: unknown; mimeType?: unknown };
  } catch {
  ...
  try {
    ...
    if (!upstream.ok || !text) {
      ...
      return errorResponse("Nexo could not understand that message. Please try again.", 502);
    }
