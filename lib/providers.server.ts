...
//
// Four Nexo model profiles use server-side OpenRouter routes with free fallbacks.
// Craft V3 alone uses the Gemini API through GEMINI_API_KEY.
...
export const PROVIDER_CONFIG: Record<NexoModelId, ProviderConfig> = {
  ...
  "craft-v3": {
    provider: "gemini",
    model: CRAFT_MODEL,
    // Craft V3 is critical; if Gemini 3.5 Flash is busy, we fallback to high-quality free coding models on OpenRouter
    fallbackModels: ["poolside/laguna-s-2.1:free", "cohere/north-mini-code:free", "nvidia/nemotron-3-ultra-550b-a55b:free"],
